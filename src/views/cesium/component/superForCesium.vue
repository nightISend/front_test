...
declare const Cesium: any; //避免找不到名称“SuperMap3D”报错
// (window as any).CESIUM_BASE_URL = "public/Cesium/Cesium.js";
...
function loadMap() {
  ...
  //天地图矢量底图
  var tianDiTuVector = new Cesium.WebMapTileServiceImageryProvider({
    url:
  ...
  //天地图矢量注记
  var tianDiTuVectorAnnotation = new Cesium.WebMapTileServiceImageryProvider({
    url:
  ...
  //天地图影像底图
  var tianDiTuSpatial = new Cesium.WebMapTileServiceImageryProvider({
    url:
  ...
  //天地图影像注记
  const tianDiTuSpatialAnnotation = new Cesium.WebMapTileServiceImageryProvider(
    {
      url:
  ...
  const viewerForOverView = new Cesium.Viewer("overview", {
    // terrain: Cesium.Terrain.fromWorldTerrain({
    //   requestVertexNormals: true,
    //   requestWaterMask: true
    // }),
  ...
  viewer.camera.setView({
    destination: Cesium.Cartesian3.fromDegrees(100, 30, 2000000),
    /* 默认好用 */
    orientation: {
      /*
        Roll:绕X旋转
  ...
  //   viewer.camera.flyTo({
  //     destination: Cesium.Cartesian3.fromDegrees(120, 30, 800)
  //     // orientation: {
  ...
  */
  ...
  let polygonMaterial = new Cesium.ColorMaterialProperty(
    new Cesium.Color(1.0, 1.0, 1.1, 1.0)
  );
  var addPolygon = viewer.entities.add({
    id: "rectangle",
    rectangle: {
      coordinates: Cesium.Rectangle.fromDegrees(100, 20, 105, 30),
      height: 10,
  ...
  const addLable = viewer.entities.add({
    id: "lable",
    position: Cesium.Cartesian3.fromDegrees(110.2, 34.5, 20000),
    label: {
      text: "飞机模型",
  ...
  const addRotateLable = viewer.entities.add({
    id: "RotateLable",
    position: Cesium.Cartesian3.fromDegrees(119.2, 33.5, 20000),
    // 文本不能转
    orientation: Cesium.Transforms.headingPitchRollQuaternion(
      Cesium.Cartesian3.fromDegrees(119.2, 33.5, 20000),
  ...
  document.addEventListener("keydown", e => {
    ...
    if (e.key == "w") {
      ...
      viewer.camera.lookLeft(Cesium.CesiumMath.toRadians(0.1));
    } else if (e.key == "e") {
      viewer.camera.lookRight(Cesium.CesiumMath.toRadians(0.1));
    } else if (e.key == "z") {
      viewer.camera.lookUp(Cesium.CesiumMath.toRadians(0.1));
    } else if (e.key == "c") {
      viewer.camera.lookDown(Cesium.CesiumMath.toRadians(0.1));
    }
  });
    ...
    if (pick != undefined) {
      ...
      if (typeof pick.id == "string") {
        ...
        attributes.color = Cesium.ColorGeometryInstanceAttribute.toValue(
          ...
          // 随机变化RGB
          Cesium.Color.fromRandom({ red: 0.5, green: 0.1 })
        );
