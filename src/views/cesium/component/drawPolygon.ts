...
export function click_draw_polygon(viewer) {
  // 清除可能会用到的监听事件
  if (handler) {
  ...
  temporary_polygon_entity = viewer.entities.add({
    polygon: {
      ...
      hierarchy: new CallbackProperty(() => {
        ...
        return new PolygonHierarchy(polygon_point_arr);
      }, false),
...
function draw_polygon(viewer) {
  ...
  if (polygon_point_arr.length >= 3) {
    ...
    //   polyObj.lon = Math.toDegrees(cartographic.longitude)
    //   polyObj.lat = Math.toDegrees(cartographic.latitude)
