   function onload(SuperMap3D) {
        ...
        try{
            ...
            SuperMap3D.when(promise,function(layer){
                ...
                scene.camera.setView({
                    destination : new SuperMap3D.Cartesian3(-2178804.249321984,4380733.469416426,4091924.387354936),
                    orientation : {
            ...
            while(otherLines.length > 147){
                for(var i = 0;i < startLines.length;i++){
                    ...
                    for(var j = 0;j < otherLines.length;j++){
                        ...
                        if(SuperMap3D.Math.equalsEpsilon(startPoint.x,endPoint.x,SuperMap3D.Math.EPSILON5)&& SuperMap3D.Math.equalsEpsilon(startPoint.y,endPoint.y,SuperMap3D.Math.EPSILON5) && SuperMap3D.Math.equalsEpsilon(startPoint.z,endPoint.z,SuperMap3D.Math.EPSILON5)){
                            flag = true;
        ...
        scene.postRender.addEventListener(function(){
            ...
            var lonD = SuperMap3D.Math.toDegrees(lonR).toFixed(4);
            var latD = SuperMap3D.Math.toDegrees(latR).toFixed(4);
