                ...
                if (this.canvasLayer && !this.stopAniamation) {
                    ...
                    let _tCoord = {
                        transferCoordinate: function (e) {
                            let i = [99999, 99999],
                                a = SuperMap3D.Cartesian3.fromDegrees(e[0], e[1]);
                            if (!a) return i;
                            let n = scene.cartesianToCanvasCoordinates(a);
                            if (!n) return i;
                            if (scene.mode === SuperMap3D.SceneMode.SCENE3D) {
                                if (SuperMap3D.Cartesian3.angleBetween(scene.camera.position, a) > SuperMap3D.Math.toRadians(80)) return !1
                            }
                            return [n.x, n.y]
                        }
                    };
                    ...
                    // WGS84坐标中的位置转换为窗口坐标
                    let coord = SuperMap3D.SceneTransforms.wgs84ToWindowCoordinates(scene, SuperMap3D.Cartesian3.fromDegrees(0, 0));
