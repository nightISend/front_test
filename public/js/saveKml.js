var ModelCache = {};
function createModelNode(SuperMap3D,entity){
    var model = entity.model;
    var modelNode;
    if(model){
        var position = entity.position;
        var locationNode;
        if(position instanceof SuperMap3D.SampledPositionProperty){
            ...
            var i = 0;
            while(j < len){
                points.push(new SuperMap3D.Cartesian3(values[i],values[i+1],values[i+2]));
                var time = data._times[j];
                times[j] = SuperMap3D.JulianDate.toIso8601(time);
                j++;
                i = i + 3;
            }
            var trackNodes = [];
            for(var i = 0,j = times.length;i < j;i++){
                var coord = SuperMap3D.Cartographic.fromCartesian(points[i]);
                var lon = SuperMap3D.Math.toDegrees(coord.longitude);
                var lat = SuperMap3D.Math.toDegrees(coord.latitude);
        ...
        var docUri = new SuperMap3D.Uri(document.location.href);
        var modelUri = new SuperMap3D.Uri(model._uri._value);
...
function createPlaceMark(SuperMap3D,entity,modelNode){
    var position = entity.position;
    var placeMarkNode;
    ...
    if(modelNode){
        ...
        if(position instanceof SuperMap3D.CompositePositionProperty){
            var intervals = position.intervals;
            if(intervals && intervals instanceof SuperMap3D.TimeIntervalCollection){
                var timeInterval = intervals._intervals[0];
                if(timeInterval && timeInterval instanceof SuperMap3D.TimeInterval){
        ...
        else if(position instanceof SuperMap3D.SampledPositionProperty){
            var data = position._property;
            var times = [];
            var values = data._values;
            var points = [];
            ...
            var coord = SuperMap3D.Cartographic.fromCartesian(point);
            var lon = SuperMap3D.Math.toDegrees(coord.longitude);
            var lat = SuperMap3D.Math.toDegrees(coord.latitude);
...
function write(SuperMap3D,entity){
    var styleNode;
    var modelNode;
    ...
    if(entity.billboard){
        ...
        placeMarkNode = createPlaceMark(SuperMap3D,entity,modelNode);
    }
    else if(entity.model){
        modelNode = createModelNode(SuperMap3D,entity);
        placeMarkNode = createPlaceMark(SuperMap3D,entity,modelNode);
    }
    else{
...
function writeKml(SuperMap3D,entities){
    ...
    var objs = [];
    for(var m = 0,n = entities.length;m < n;m++){
        objs.push(write(SuperMap3D,entities[m]));
    }
...
function saveKml(SuperMap3D,viewer){
    ...
    var kmlContent = writeKml(SuperMap3D,entities);
